...
@Sql("classpath:sql/states/states.sql") public class StatesRepositoryIT
    ...
    public void findTopByApplicationEntityOrderByStateIndexDescShouldNotFindStateForApplicationWithoutStates() {
        ...
        // When
        StateEntity stateEntity = this.statesRepository
            .findTopByApplicationEntityOrderByStateIndexDesc(applicationEntity);
    ...
    public void findTopByApplicationEntityOrderByStateIndexDescShouldFindThreeStateForApplicationWithThreeState() {
        ...
        StateEntity expectedStateEntity =
    ...
    private void checkSortingValidationWithThreeElements(List<StateEntity> stateEntityList) {
        List<StateEntity> expectedStateEntityList =
    ...
    private StateEntity getExpectedStateEntityForSingleState() {
        return StateEntity.builder().applicationEntity(
            getExpectedApplicationEntity(CANDIDATE_NAME_B, FIRST_ID, CHANNEL_NAME_DIRECT, FIRST_ID,
                POSITION_NAME_DEVELOPER)).stateIndex(VALUE_ONE).build();
    }

    private List<StateEntity> getExpectedSortingStateEntityListWithThreeElements() {
        List<StateEntity> stateEntityList = Arrays.asList(StateEntity.builder().applicationEntity(
            getExpectedApplicationEntity(CANDIDATE_NAME_C, FOURTH_ID, CHANNEL_NAME_FACEBOOK,
                FIRST_ID, POSITION_NAME_DEVELOPER)).stateIndex(BIGGEST_STATE_INDEX_NUMBER).build(),

            StateEntity.builder().applicationEntity(
                getExpectedApplicationEntity(CANDIDATE_NAME_C, FOURTH_ID, CHANNEL_NAME_FACEBOOK,
                    FIRST_ID, POSITION_NAME_DEVELOPER)).stateIndex(MIDDLE_STATE_INDEX_NUMBER)
                .build(), StateEntity.builder().applicationEntity(
                getExpectedApplicationEntity(CANDIDATE_NAME_C, FOURTH_ID, CHANNEL_NAME_FACEBOOK,
                    FIRST_ID, POSITION_NAME_DEVELOPER)).stateIndex(SMALLEST_STATE_INDEX_NUMBER)
                .build());
    ...
    private ApplicationEntity getApplicationByCandidateName(String candidateName) {
        ...
        return this.applicationsRepository.findByCandidateEntity(candidateEntity)
            .get(ZEROTH_ELEMENT);
    }

    private ApplicationEntity getExpectedApplicationEntity(String candidateName, Long channelId,
        String channelName, Long positionId, String positionName) {
        CandidateEntity candidateEntity = this.candidateRepository.findByName(candidateName);

        ChannelEntity expectedChannelEntity =
            ChannelEntity.builder().id(channelId).name(channelName).build();

        PositionEntity expectedPositionEntity =
            PositionEntity.builder().id(positionId).name(positionName).build();
        ...
        ApplicationEntity expectedApplicationEntity =
            ApplicationEntity.builder().id(expectedApplicationId)
                .positionEntity(expectedPositionEntity).channelEntity(expectedChannelEntity)
                .candidateEntity(candidateEntity).creationDate(currentDate).build();
    ...
    private void assertApplicationEntity(ApplicationEntity application,
        ...
        assertThat(application.getChannelEntity(),
            is(expectedApplicationEntity.getChannelEntity()));
        ...
        assertThat(application.getPositionEntity(),
            is(expectedApplicationEntity.getPositionEntity()));
