...
@Service @Slf4j public class CandidateServiceImpl implements CandidateService {
    ...
    @Override public CandidateDTO getCandidate(Long id) {
        Assert.notNull(id);
    ...
    @Override public Collection<CandidateDTO> getAllCandidate(FilterRequest sortingRequest) {
        ...
        Collection<CandidateEntity> candidateEntities = candidateRepository
            .findAllByNameContainingAndEmailContainingAndPhoneContaining(searchOptions.getName(),
                searchOptions.getEmail(), searchOptions.getPhone(), sort);

        return modelMapper.map(candidateEntities, CANDIDATEDTO_LIST_TYPE);
    }

    @Override public Long saveOrUpdate(CandidateDTO candidate) {
    ...
    @Override public CandidateDTO getCandidateByName(String name) {
