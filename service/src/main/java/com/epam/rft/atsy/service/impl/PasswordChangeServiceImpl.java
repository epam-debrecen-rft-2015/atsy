...
@Service @Slf4j public class PasswordChangeServiceImpl implements PasswordChangeService {
    ...
    @Override public Long saveOrUpdate(PasswordHistoryDTO passwordHistoryDTO) {
        Assert.notNull(passwordHistoryDTO);
        Assert.notNull(passwordHistoryDTO.getUserId());
        PasswordHistoryEntity entity =
            modelMapper.map(passwordHistoryDTO, PasswordHistoryEntity.class);
        ...
        try {
            ...
            throw new DuplicateRecordException(userId,
                "Duplication occurred when saving password history for user with ID: " + userId,
                ex);
        }
    }

    @Override public List<String> getOldPasswords(Long userId) {
    ...
    @Override public void deleteOldestPassword(Long userId) {
        Assert.notNull(userId);
        PasswordHistoryEntity entity =
