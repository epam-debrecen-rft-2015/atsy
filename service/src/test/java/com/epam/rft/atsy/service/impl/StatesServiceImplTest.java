...
@RunWith(MockitoJUnitRunner.class) public class StatesServiceImplTest {
    ...
    private static final ApplicationEntity APPLICATION_ENTITY_WITHOUT_STATES =
    ...
    private static final List<ApplicationEntity> EMPTY_APPLICATION_ENTITY_LIST =
    ...
    private final CandidateEntity firstCandidateEntity =
        CandidateEntity.builder().id(FIRST_ID).build();
    ...
    private final StateEntity firstStateEntity =
        StateEntity.builder().id(FIRST_ID).applicationEntity(firstApplicationEntity)
            .stateType(FIRST_STATE_TYPE_NAME).creationDate(presentDate).build();
    ;
    private final StateEntity secondStateEntity =
        StateEntity.builder().id(SECOND_ID).applicationEntity(secondApplicationEntity)
            .stateType(SECOND_STATE_TYPE_NAME).creationDate(presentDate).build();
    private final StateEntity thirdStateEntity =
        StateEntity.builder().id(THIRD_ID).applicationEntity(thirdApplicationEntity)
            .stateType(THIRD_STATE_TYPE_NAME).creationDate(presentDate).build();
    private final StateEntity savedStateEntity =
        StateEntity.builder().id(FIRST_ID).applicationEntity(secondApplicationEntity)
            .stateType(FIRST_STATE_TYPE_NAME).creationDate(presentDate).build();
    ...
    private final List<StateEntity> stateEntityListWithSingleElement =
        Arrays.asList(firstStateEntity);
    private final List<StateEntity> stateEntityListWithThreeElements =
        Arrays.asList(firstStateEntity, secondStateEntity, thirdStateEntity);
    private final List<StateViewDTO> stateViewDTOListWithSingleElement =
        Arrays.asList(stateViewDTO);
    private final List<StateViewDTO> stateViewDTOListWithThreeElements =
        Arrays.asList(stateViewDTO, stateViewDTO, stateViewDTO);


    private List<CandidateApplicationDTO> candidateApplicationDTOListWithSingleElement = Arrays
        .asList(CandidateApplicationDTO.builder().applicationId(FIRST_ID)
            .stateType(FIRST_STATE_TYPE_NAME).positionName(FIRST_POSITION_ENTITY_NAME)
            .lastStateId(FIRST_ID).creationDate(SIMPLE_DATE_FORMAT.format(presentDate))
            .modificationDate(SIMPLE_DATE_FORMAT.format(presentDate)).build());

    private List<CandidateApplicationDTO> candidateApplicationDTOListWithThreeElements = Arrays
        .asList(CandidateApplicationDTO.builder().applicationId(FIRST_ID)
                .stateType(FIRST_STATE_TYPE_NAME).positionName(FIRST_POSITION_ENTITY_NAME)
                .lastStateId(FIRST_ID).creationDate(SIMPLE_DATE_FORMAT.format(presentDate))
                .modificationDate(SIMPLE_DATE_FORMAT.format(presentDate)).build(),
            ...
            CandidateApplicationDTO.builder().applicationId(SECOND_ID)
                ...
                .lastStateId(SECOND_ID).creationDate(SIMPLE_DATE_FORMAT.format(presentDate))
                .modificationDate(SIMPLE_DATE_FORMAT.format(presentDate)).build(),

            CandidateApplicationDTO.builder().applicationId(THIRD_ID)
                .stateType(THIRD_STATE_TYPE_NAME).positionName(THIRD_POSITION_ENTITY_NAME)
                .lastStateId(THIRD_ID).creationDate(SIMPLE_DATE_FORMAT.format(presentDate))
                .modificationDate(SIMPLE_DATE_FORMAT.format(presentDate)).build());
    ...
    @InjectMocks private StatesServiceImpl statesService;
    ...
    @Test public void getStatesByApplicationIdShouldReturnAnEmptyList() {
        // Given
        given(applicationsRepository.findOne(FIRST_ID))
            .willReturn(APPLICATION_ENTITY_WITHOUT_STATES);
    ...
    @Test public void getStatesByApplicationIdShouldReturnAListWithSingleElement() {
        // Given
        ...
        given(statesRepository.findByApplicationEntityOrderByStateIndexDesc(firstApplicationEntity))
            .willReturn(stateEntityListWithSingleElement);
        given(modelMapper.map(stateEntityListWithSingleElement, STATE_VIEW_DTO_LIST_TYPE))
    ...
    @Test public void getStatesByApplicationIdShouldReturnAListWithThreeElement() {
        // Given
        given(applicationsRepository.findOne(FIRST_ID)).willReturn(firstApplicationEntity);
        given(statesRepository.findByApplicationEntityOrderByStateIndexDesc(firstApplicationEntity))
            .willReturn(stateEntityListWithThreeElements);
        given(modelMapper.map(stateEntityListWithThreeElements, STATE_VIEW_DTO_LIST_TYPE))
    ...
    @Test public void saveStateShouldBeSuccessSaving() {
        // Given
    ...
    @Test public void getCandidateApplicationsByCandidateIdShouldReturnAnEmptyList() {
        // Given
        given(candidateRepository.findOne(FIRST_ID))
            .willReturn(CANDIDATE_ENTITY_WITHOUT_APPLICATIONS);
    ...
    @Test public void getCandidateApplicationsByCandidateIdShouldReturnAListWithSingleElement() {
        // Given
    ...
    @Test public void getCandidateApplicationsByCandidateShouldReturnAListWithThreeElements() {
        // Given
        given(candidateRepository.findOne(FIRST_ID)).willReturn(firstCandidateEntity);
        given(applicationsRepository.findByCandidateEntity(firstCandidateEntity))
    ...
    private void assertStateViewDtoListByCreationDateAsString(List<StateViewDTO> stateViewDTOList) {
        for (StateViewDTO stateViewDTO : stateViewDTOList) {
            assertThat(stateViewDTO.getCreationDate(),
    ...
    private void assertStateEntityWhenSavingStateEntity(StateEntity stateEntity,
        StateEntity expectedSavedStateEntity) {
        assertThat(stateEntity, notNullValue());
        assertThat(stateEntity.getApplicationEntity(),
            equalTo(expectedSavedStateEntity.getApplicationEntity()));
        assertThat(stateEntity.getCreationDate(),
            greaterThanOrEqualTo(expectedSavedStateEntity.getCreationDate()));
    }
