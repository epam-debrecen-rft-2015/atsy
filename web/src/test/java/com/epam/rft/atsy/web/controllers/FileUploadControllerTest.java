...
import com.epam.rft.atsy.web.controllers.rest.FileUploadController;
...
/*
  ...
  public void uploadFileShouldSaveFileWhenEverythingIsOk() throws Exception {
    MockMultipartFile multipartFile = MultipartFileCreatorTestHelper.createMultipartFile(ORIGINAL_FILENAME_VALID, FILE_SIZE_HUNDRED_BYTE);
    given(candidateService.getCandidate(CANDIDATE_ID)).willReturn(candidateDTOWithNullCVFilename);
    given(folderHandler.createCVFileFromFolderLocationAndCandidateDtoAndCVFilename(CV_TEST_FOLDER_LOCATION_PATH, CANDIDATE_ID, ORIGINAL_FILENAME_VALID)).willReturn(cvFile);
    this.mockMvc.perform(buildFileUploadRequest(multipartFile))
        .andExpect(status().is3xxRedirection())
    ...
    then(folderHandler).should().createCVFileFromFolderLocationAndCandidateDtoAndCVFilename(CV_TEST_FOLDER_LOCATION_PATH, CANDIDATE_ID, ORIGINAL_FILENAME_VALID);
